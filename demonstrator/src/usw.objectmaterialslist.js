...
Classes	: usw.objectmaterialslist.js
Summary	: List of AAT materials linked to objects in the dataset
Require	: jquery-2.X.X.min.js, jquery-ui.min.js, usw.basecontrol.js, fn.sparqlquery.js
Example	: $("#div1").objectmaterials();
...
;(function ($) {            // start of main jquery closure
    ...
    $.widget('usw.objectmaterialslist', $.usw.resourcelist, {
        options: {
            language: "en",
            //headertext: "Object Materials",
            collapsible: false,
            showheader: false,
            showcontent: true,
            showfooter: false
        ...
        _create: function (options) {
            var self = this;
            // override default options with any passed in values, call super 
            self.options = $.extend({}, self.options, options);
            self._super(self.options);

            $(self.element).addClass("usw-object-materials");

            // query to get all (AAT) materials linked to a resource via crm:P45_consists_of
            var sparql = "SELECT ?uri STR(?lbl) AS ?label COUNT(?uri) AS ?counter WHERE { \n";
            sparql += "?obj a <" + usw.uri.CRM.E22 + ">;  <" + usw.uri.CRM.P45 + "> ?uri .\n";
